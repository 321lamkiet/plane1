...
# ==========================================
st.set_page_config(
    page_title="TikTok Intelligence OS v6.2",
    page_icon="🌍",
    layout="wide",
...
class DatabaseEngine:
    def __init__(self, db_name="tiktok_v62_global.db"): 
...
def check_market_hours(country_code):
    try:
        # Nếu là ALL (Global) thì luôn Active
        if country_code == "ALL":
            return True, "Toàn Cầu (24/7)"
...
def run_apify_scan(token, hashtags_str, limit, country_code, filter_mode, ai_keywords, custom_proxy=None):
    ...
    if custom_proxy and custom_proxy.strip():
        ...
        # [NEW] Logic xử lý "Tất cả"
        if country_code == "ALL":
             proxy_config = {"useApifyProxy": True} # Để trống quốc gia -> Apify tự chọn
        else:
...
# [FIX] TÍNH TOÁN VIRAL RATIO TRƯỚC KHI LỌC
if not df_all.empty:
