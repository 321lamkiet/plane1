...
# ==========================================
st.set_page_config(
    page_title="TikTok OS v7.0 (Rescue)",
    page_icon="🚑",
    layout="wide",
...
class DatabaseEngine:
    def __init__(self, db_name="tiktok_v70_rescue.db"): 
    ...
    def upsert_video(self, data, velocity, v_type):
        now = datetime.now()
        vid_id = data.get('id')
        if not vid_id: return # Bỏ qua nếu không có ID
...
def analyze_with_gemini(api_key, video_desc, author):
    ...
    # [FIX] Danh sách model dự phòng
    models = ['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-pro']
    
    for m in models:
        try:
            model = genai.GenerativeModel(m)
            response = model.generate_content(f"Phân tích Marketing TikTok: Tác giả {author}, Nội dung: {video_desc}. 3 ý: Hook, Nỗi đau, Remake.")
            return response.text
        except:
            continue
    return "Lỗi AI: Không model nào hoạt động. Hãy kiểm tra API Key."

# ==========================================
# 5. GIAO DIỆN (FRONTEND)
# ==========================================
with st.sidebar:
    st.title("🦅 TikTok OS v7.0")
    st.caption("Rescue Edition")
...
# [FIX] Xử lý dữ liệu an toàn để tránh lỗi ValueError
if not df_all.empty:
    # Ép kiểu dữ liệu sang số, lỗi biến thành 0
...
def render_list(df, is_saved=False):
    ...
    for _, row in df.head(50).iterrows():
        ...
        with st.expander(f"{emoji} {row['velocity_value']:.0f}/h | {row['author_name']}"):
            ...
            if st.button("🧠 Phân tích AI", key=f"ai_{row['video_id']}_{is_saved}"):
                with st.spinner("AI đang đọc..."):
                    anl = analyze_with_gemini(gemini_key, row['description'], row['author_name'])
...
with tab3:
    if not df_all.empty:
        try:
            # [FIX] Bỏ log_x=True để tránh lỗi log(0)
            fig = px.scatter(
                ...
                title="Rađa Viral (Kênh nhỏ View to nằm bên trái)"
            )
