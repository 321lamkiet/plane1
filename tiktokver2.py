...
# ==========================================
st.set_page_config(
    page_title="TikTok Intelligence OS v6.6 (Final)",
    page_icon="🚀",
    layout="wide",
...
class DatabaseEngine:
    def __init__(self, db_name="tiktok_v66_final.db"): 
...
def check_market_hours(country_code):
    try:
        if country_code == "ALL": return True, "Toàn Cầu (24/7)"
        if country_code == "VN": tz = pytz.timezone('Asia/Ho_Chi_Minh')
...
def analyze_with_gemini(api_key, video_desc, author):
    ...
    try:
        ...
        # Sử dụng Model mới nhất, nhanh nhất
...
# [FIX] Đảm bảo dữ liệu an toàn cho biểu đồ
if not df_all.empty:
...
def render_video_list(dataframe, is_saved_tab=False):
    ...
    for _, row in dataframe.head(50).iterrows(): 
        ...
        with st.expander(card_title, expanded=False):
            ...
            search_query = urllib.parse.quote(row['description'][:50]) 
            col_a, col_b = st.columns(2)
            with col_a:
                st.markdown(f"[🔍 Amazon](https://www.amazon.com/s?k={search_query})", unsafe_allow_html=True)
            with col_b:
                st.markdown(f"[🛍️ AliExpress](https://www.aliexpress.com/wholesale?SearchText={search_query})", unsafe_allow_html=True)
            st.markdown("---")
...
with main_tab2:
    if df_saved.empty:
        st.warning("Bạn chưa lưu video nào.")
    else:
        render_video_list(df_saved, is_saved_tab=True)

with main_tab3:
    if not df_all.empty and 'viral_ratio' in df_all.columns:
