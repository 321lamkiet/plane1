...
# ==========================================
st.set_page_config(
    page_title="TikTok Intelligence OS v4.5",
    page_icon="🦅",
    layout="wide",
...
with st.sidebar:
    ...
    # --- [NEW] CẤU HÌNH TỰ ĐỘNG ---
    auto_mode = st.checkbox("Bật chế độ chạy tự động (Loop)")
    
    # Chỉ hiện các tùy chọn này nếu bật Auto
    if auto_mode:
        scan_interval = st.number_input("Thời gian nghỉ giữa các lần (Phút)", min_value=15, max_value=120, value=30, step=5, help="Ví dụ: 60 phút quét 1 lần để tiết kiệm tiền")
        max_scans = st.number_input("Giới hạn số lần quét/ngày", 1, 50, 10)
    else:
        st.info("Chế độ thủ công: Quét 1 lần rồi dừng.")
        scan_interval = 30 # Giá trị mặc định ẩn
        max_scans = 1 # Chạy 1 lần
    # -----------------------------
...
if not df.empty:
    ...
    scan_status = f"{st.session_state['scan_count']}/{max_scans}" if auto_mode else "Manual Mode"
    ...
    if auto_mode:
        if st.session_state['scan_count'] >= max_scans: 
            st.toast("🛑 Đã đạt giới hạn quét. Dừng Auto.", icon="🛑")
        elif not market_active:
            st.toast(f"💤 Thị trường Mỹ đang ngủ ({us_time}). Tạm dừng...", icon="💤")
            # Ngủ 30 phút rồi check lại giờ
            time.sleep(1800) 
            st.rerun()
        else:
            # [QUAN TRỌNG] Chuyển đổi phút sang giây
            sleep_seconds = scan_interval * 60 
            st.toast(f"⏳ Đang chờ {scan_interval} phút cho lần quét tiếp theo...", icon="⏳")
            
            # Ngủ đúng thời gian bạn cài đặt
